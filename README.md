### Hi, I’m Thushar 👋

Backend Software Engineer working with **Java (Spring Boot)** and **C# (.NET)**.  
I focus on building **clean, scalable APIs** with strong domain boundaries and maintainable architecture.

📍 Karnataka, India  

<a href="https://www.linkedin.com/in/thushxr" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-Connect-blue?logo=linkedin" />
</a>
<a href="https://www.instagram.com/thushxr" target="_blank">
  <img src="https://img.shields.io/badge/Instagram-Follow-pink?logo=instagram" />
</a>
<a href="https://twitter.com/thushxr" target="_blank">
  <img src="https://img.shields.io/badge/Twitter-Follow-black?logo=x" />
</a>

---

### Tech

<p>
  <img src="https://skillicons.dev/icons?i=java,cs,dotnet,spring,mysql,aws,git,grafana,html,css,js,idea,ai,ps,postman,postgres,redis,rider,figma" />
</p>

---

I enjoy working on backend systems that are **simple, correct, and easy to evolve**.
